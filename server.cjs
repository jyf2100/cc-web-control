...
function startWebServer() {
  ...
  const requireAuth = (req, res, next) => {
    ...
    // theme.js 必须公开:同理,它是 login.html 引用的主题切换脚本(无 tmux 控制面)。
    // These do not grant access to tmux control and help avoid confusing stale icons in browsers.
    if (p === '/login' || p === '/healthz' || p === '/logo.png' || p === '/favicon.ico' || p === '/manifest.json' || p === '/tokens.css' || p === '/theme.js' || p === '/icon-192.png' || p === '/icon-512.png' || p === '/apple-touch-icon.png') return next();
